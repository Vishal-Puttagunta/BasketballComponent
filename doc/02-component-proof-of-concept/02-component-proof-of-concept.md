...
I chose to move forward with my mock Library transaction interface for a multitude of reasons. I could have kernel methods such as addItem(), borrowItem(), returnItem(), isAvailable(). While also having secondary methods that build off of them such as searchByAuthor(), getInventoryCount(), getWaitlist(). These can also be expanded into other categories such as movies or music and be expanded into multiple categories of items you can checkout at the library.
